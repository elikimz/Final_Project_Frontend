...
const FleetManagementPage: React.FC = () => {
    ...
    if (storedVehicleId) {
      ...
      if (!isNaN(vehicleId)) {
        setFormData((prevData) => {
          console.log('Form data with vehicleId from localStorage:', { ...prevData, vehicleId });
          return { ...prevData, vehicleId };
        });
      } else {
  ...
  const handleEditClick = (item: any) => {
    ...
    setFormData({
      vehicleId: item.vehicle_id, // Adjusted to match formData
      status: item.status,
